...
def reducer():
    ...
    try:
        ...
        l = len(reducer_input)
        reducer_output = {}
        for i, word_dict in enumerate(reducer_input):
            reducer_progress = ((i + 1) / l) * 100
            # print(i+1, l, reducer_progress)
            for word in word_dict:
                if word not in reducer_output:
                    reducer_output[word] = []
                reducer_output[word] += word_dict[word]
            # reducer_output[pair[0]] = reducer_output.get(pair[0], 0) + 1
        # print(f"Reducer {reducer_id} output: {reducer_output}")
